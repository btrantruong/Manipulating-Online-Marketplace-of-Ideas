...
from experiment_archive.config_values import THETA, GAMMA, PHI_LIN, BETA
...
pprint = {
    ...
    "beta": "$\\beta$",
    "phi": "$\phi$",
    "quality": "Relative Average Quality",
...
def combine_results(res_dir, res_folders, prefix=None):
    ...
    all_results = {
        fname: json.load(open(os.path.join(run1, "%s.json" % fname), "r"))
...
def val2expname(
    all_results, all_configs, value=("theta", 1), exp_type="vary_thetagamma"
):
    ...
    for expname, params in config.items():
        if params[param] == val:
            ...
            results += [
                utils.update_dict(res, params)
            ]  # fill in missing params with config
    return results
...
def lineplot_data(
    ...
    multiline_data = (
        []
    )  # list of tuple, data=[(x1,y1, err2), (x2,y2, err2)] - y is the average over multiple runs

    # TODO: Relative data compared to targeting
    for line_val in line_values:
        data = val2expname(
            all_results, all_configs, value=(line_name, line_val), exp_type=exp_type
        )

        x = [results[x_name] for results in data]  # array of
        if y_name == "discriminative_pow":
            y = [
                results[y_name][0] for results in data
            ]  # since discrinminative_pow is a tuple of (tau,p_val)
        else:
        ...
        x_y_err = zip(
            *sorted(zip(x, y_avg, y_err))
        )  # sort data based on x, results in a zip object
...
def draw_lines(
    ax, data, line_values, line_name="beta", x_name="beta", y_name="quality", log_x=True
):
    ...
    for idx, linedata in enumerate(data):
        ...
        ax.plot(
            ...
            y,
            marker=markers[idx],
            label="%s: %s" % (pprint[line_name], line_values[idx])
            if line_values[idx] is not None
            else "None",
        )
        ax.fill_between(
            x,
...
def plot_scatter(
    ax, exp_name, data_path, result_path, folders, exp_prefix=None, y_name="quality"
):
    ...
    multiline_data = lineplot_data(
        all_results,
        all_configs,
        exp_type=exp_name,
        **utils.remove_illegal_kwargs(config, lineplot_data)
    )
...
def heatmap_data(
    ...
    # Return data: stacked np arrays shape (n,m) where each row contains result of varying m values while keeping one parameter fixed
    ...
    results = defaultdict(
        lambda: []
    )  # each key is a row of yvals. to be converted to a 2-D array later
    ...
    for idx, yval in enumerate(yvals):
        for xval in xvals:
            cell_res = [
                ...
                if res[x_name] == xval and res[y_name] == yval
            ]  # result for one exp
            ...
            if len(cell_res) == 1:
                ...
                vals = (
                    ...
                    else exp_res[cell_type][0]
                )
            else:
                vals = [0]  # if we don't have results yet

            results[yval] += [
                np.mean(vals) if len(vals) > 0 else 0
