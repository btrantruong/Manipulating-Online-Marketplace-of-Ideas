...
def plotmulti_infosys(result_dir, exp_type='vary_betagamma', anchors=[('gamma', 0.01), ('gamma', 0.02)], 
    ...
    colormap = cm.get_cmap('inferno', 6)
...
#         data = [(info[x], info[y]) for info in results.values() if info[anchor[0]]==anchor[1]]
        ...
        plt.plot(*zip(*sorted(avg_data)),label=anchor[1] if anchor[1] is not None else 'None', color=colors[i],marker=markers[i])
    ...
    plt.ylim(bottom=0)
    plt.legend()
    plt.savefig(os.path.join(PLOT_PATH, '%s%s.png' %(x,y)), dpi=300)
    # plt.show()
    plt.clf()
...
plots = [{'exp_type': 'vary_targetgamma', 'anchors': anchors, 'x': 'gamma', 'y':'quality','log_flag':True},
        {'exp_type': 'vary_targetgamma', 'anchors': anchors, 'x': 'gamma', 'y':'discriminative_pow','log_flag':True, 'relative':False},
        {'exp_type': 'vary_targetgamma', 'anchors': anchors, 'x': 'gamma', 'y':'diversity','log_flag':True, 'relative':False},
        ]
