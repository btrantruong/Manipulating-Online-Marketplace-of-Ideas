...
#
def simulation_step(
    G, count_forgotten_memes=False, track_meme=False, alpha=15, mu=0.75, phi=1, theta=1
):
...
def add_avq_to_net(G):
    ...
    for agent in newG.nodes:
        ...
        newG.nodes[agent]["feed"] = float(
            statistics.mean([m[0] for m in G.nodes[agent]["feed"]])
        )
    return newG
...
def simulation(
    ...
    while quality_diff > epsilon:
        ...
        for _ in range(n_agents):
            num_memes, hum_memes = simulation_step(
                network,
        ...
        quality_diff = (
            abs(new_quality - old_quality) / old_quality if old_quality > 0 else 0
        )
