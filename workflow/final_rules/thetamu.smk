...
exp_type = 'vary_thetamu'

EXPS = json.load(open(config_fname,'r'))[exp_type]
# get names for exp_config and network
# exclude theta[0]=1 (default value, in which case results are same as No targeting vary_mu)
# exclude mu[1]=0.5 (default value, in which case results are same as No targeting vary_theta)
EXP_NOS = [
    exp_name
    for exp_name in EXPS.keys()
    if int(exp_name[0]) != 0 and int(exp_name[1]) != 1
]
